...
from .pipeline_state import pipeline_schedule_is_enabled
...
class _Plombery:
    ...
    def start(self):
        # Skip database setup if explicitly disabled
        if os.getenv("SKIP_DB_SETUP", "false").lower() != "true":
            setup_database()
        else:
            _logger.info("Skipping database setup (SKIP_DB_SETUP=true)")
...
def register_pipeline(
    ...
    pipeline = Pipeline(
        ...
        schedule_enabled=pipeline_schedule_is_enabled(id),
    )
